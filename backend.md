...
| `Campus` | `id, name, city` |
| `Test` | `id, title, kind: CharField(choices=[subjectWiseGuessPaper,subjectWiseSimplePaper,chapterWise])`, `board_class: FK`, `subject: FK`, `chapter: FK(null=True)`, `is_live: BooleanField(default=False)`, `live_date: DateTimeField(null=True)`, `is_free_sample: BooleanField(default=False)`, `created_by_admin: FK(Admin)`, `created_at`, `question_count: IntegerField(default=0)`, `duration_minutes: IntegerField(default=0)`, `total_marks: IntegerField(default=0)` — **no `campus` FK, no `price` field** (confirmed absent on the Flutter entity both times). The three stat fields (`question_count`/`duration_minutes`/`total_marks`) are **denormalized**: derive them server-side from the related `Question` set on save (they back the test-list card meta row + result screen; matches the new `Test` entity/DTO fields) |
| `Question` | `id, test: FK(Test), chapter: FK(Chapter), type: CharField(choices=[mcq,shortAnswer,longAnswer])`, `question_text`, `options: JSONField(null=True)`, `correct_option_index: IntegerField(null=True)`, `expected_answer: TextField(null=True)`, `marks: IntegerField(default=1)` (feeds the per-section marks breakdown on the results screen; MCQ default 1, short/long carry more) |
...
| `SubjectEnrollment` | `student: FK, subject: FK, teacher: FK(null=True), discount_applied: BooleanField(default=False), is_paid: BooleanField(default=False)` — **app-layer invariant, not just a DB default**: `discount_applied` must only ever be `True` when `teacher_id is not None` (enforce with a `CheckConstraint`, not just serializer validation). `is_paid` is set to `True` upon successful checkout of the subject pack. |
...
| `Notification` | `id, recipient_id: CharField` (polymorphic — not a real FK since it can point at Teacher/Student/Salesman/Admin), `recipient_role: CharField(choices=[admin,teacher,student,salesman])`, `type: CharField(choices=[studentRegistered,profileUpdatePending,teacherAwaitingApproval,phoneRecoveryRequested,accountDeletionRequested,studentPurchasedPack,teacherPayoutRequested,refundRequested,paymentFailedAlert,salesmanSeededTeacher,questionFlagged,liveTestCompleted,otpLockoutTriggered,roleViolationAttempt,earningsCredited,studentEnrolled,profileUpdateApproved,payoutDisbursed,newTestUploaded,discountAnnouncement,liveTestReminder,freeAttemptsExhausted,paymentSuccessful,salesmanCommissionCredited,teacherJoinedWithCode])`, `message: TextField, is_read: BooleanField(default=False), created_at` |
| `Cart` | `id, student: OneToOne, total_amount: DecimalField(default=0)` (`items` = reverse FK from `CartItem`) |
| `CartItem` | `cart: FK, subject: FK(Subject), price: DecimalField, discounted_price: DecimalField(null=True)` (Subject bundle purchasing — students buy an entire subject pack, not single tests) |
...
| `/teachers` | GET | List/filter approved teachers by campus. Query parameters (strictly literal camelCase): `?campusId={campusId}` (optional: `&subjectId={subjectId}`). Requires `IsAuthenticated`. Returns `TeacherOptionDto[]` (`id`, `name`, `campusId`, `subjectIds`, `classIds`). Always hardcodes filter to approved teachers only (do NOT pass `approvalStatus`). Excludes soft-deleted teachers. |
...
| `/students/{id}/subject-enrollments` | GET / PUT | GET = reads current subject/teacher enrollments (`SubjectEnrollment[]`); PUT = persists onboarding and post-onboarding subject/teacher selections with array of `{"subjectId": string, "teacherId": string?}`. Backend preserves `isPaid: true` for previously purchased subjects, and automatically sets `discountApplied: true` server-side whenever `teacherId` is present. `SubjectEnrollmentDto` includes `isPaid: bool`. |
| `/students/{id}/test-attempts` | GET | Every `TestAttempt` for this student, newest first (`student-progress`, `teacher-students`' per-student detail). Also backs the **free-attempts gate** (see note below) |

> **Free-attempts limit (Phase-1, client-side).** A student gets `AppConfig.freeAttemptsPerStudent` (currently **5**) free test attempts total across any test/chapter. "Remaining attempts" is derived **client-side** as `limit − count(this student's TestAttempts)` from the `/students/{id}/test-attempts` response above — **no new endpoint is required for Phase-1**. Once exhausted, the app shows an upgrade/payment overlay and locks further attempts until the purchase gate (`/payments/status`) clears. Phase-2 hook: if server-enforced gating is ever needed, expose a per-student entitlement/remaining-attempts flag (parallels the "backend must enforce its own OTP lockout" note below — a client-only limit is trivially bypassed). No `TestAttempt`/`SubmissionAnswer` shape changes: the results section-breakdown + review sheet are computed client-side from existing fields.
...
| `/payments/status` | GET | `{"purchasedSubjectIds": string[]}` — backing the purchase gate; derived server-side from successful `Payment` rows for purchased subject packs. Query parameter `?studentId={studentId}` is optional and defaults to the authenticated caller from the JWT token. |
...
- **Rate-limit/lockout**: the Flutter side has client-facing constants for this shape (`AppConfig.otpResendCooldownSeconds`, `otpMaxResendAttempts`) that only drive the OTP bottom sheet's resend-cooldown UX (`OtpTimerViewModel`) — they are not enforced anywhere and are trivially bypassed (app restart). The backend needs its own real enforcement (e.g. `django-ratelimit` or a Redis-backed attempt counter keyed by phone number). See `backend_otp_rate_limiting.md` for the full spec, including the exact `429` response contract the client (`ErrorInterceptor`) is already wired to consume with no Flutter changes needed.
